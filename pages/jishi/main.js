...
async function setPositingDataInFirstPage(that, pageNo = 1, keyword, labelId, timeIndex) {
  ...
  // 2.判断、请求、处理下一页数据
  try {
    const res = await getPostingWithPage(pageNo, keyword, labelId, timeIndex);
    const firstPageData = dealWithPostingAndError(res);
...
async function getNextPostingPage(that, keyword, labelId, timeIndex) {
  ...
  if (!isLastPage) {
    // 没到最后一页，再发送请求
    try {
      const res = await getPostingWithPage(current + 1, keyword, labelId, timeIndex);
      const nextPageData = dealWithPostingAndError(res);
      // setData是page对象里才有的办法，所以在调用函数时，要把page对象传入进来；
...
Page({
  ...
  onLoad: function (options) {
    ...
    request({
      ...
      .then(res => {
        ...
        // 用map从json中提取标签列表
        const result = res.data.data.map(data => data.content);
  ...
  jumpToDetail: function (e) {
    ...
    wx.navigateTo({
      url:
        "/pages/jishi/detail-new?postingId=" +
        e.currentTarget.dataset.postingid +
        "&index=" +
  ...
  clickLabel: function (e) {
    ...
    const postingLabels = this.data.postingLabels;
    //    若该项已被选中，则取消选中
  ...
  getNextPage() {
    ...
    if (!isLastPage) {
      // 没到最后一页
      const { isLastPage, current, pages, jishiItemList: nextlist } = that.data.nextPageData;
