...
Page({
  ...
  getList(self) {
    ...
    Promise.all([favouritePosting, favouriteTeam])
      .then(result => {
        // 处理收藏组队的数据------------------
        const jirenItemList = result[1].data.data.map(v => {
          const team = {
        ...
        // 处理收藏帖子的数据---------------------------------
        const jishiItemList = result[0].data.data.map(v => {
          const tempList = {
            labelText: v.labelContent,
            title: v.title,
            description: v.brief,
          ...
          const updateTime = new Date(v.updateTime);
          // 毫秒单位转换
          const hour = Math.floor((now - updateTime) / 1000 / 60 / 60);
          if (hour < 24) {
  ...
  onPostingReachBottom() {
    const that = this;
    const jishiIsLastPage = that.data.jishiIsLastPage;
    if (!jishiIsLastPage) {
      let { jishiIsLastPage, jishiCurrent, jishiPages, jishiItemList } = that.data.postingNextPage;
      const before = that.data.jishiItemList;
  ...
  onTeamReachBottom() {
    const that = this;
    const jirenIsLastPage = that.data.jirenIsLastPage;
    if (!jirenIsLastPage) {
      let { jirenIsLastPage, jirenCurrent, jirenPages, jirenItemList } = that.data.teamNextPage;
      const before = that.data.jirenItemList;
...
function dealWithPosting(res) {
  if (res.statusCode >= 200 && res.statusCode <= 300) {
    const { current: jishiCurrent, pages: jishiPages, records } = res.data.data;
    let jishiIsLastPage = false;
    const jishiItemList = records.map(v => {
      const tempList = {
        labelText: v.labelContent,
        title: v.title,
        description: v.brief,
...
function dealWithTeam(res) {
  if (res.statusCode >= 200 && res.statusCode <= 300) {
    const { current: jirenCurrent, pages: jirenPages, records } = res.data.data;
    let jirenIsLastPage = false;
    const jirenItemList = records.map(v => {
      const team = {
        labelText: v.labelContent,
        title: v.title,
        description: v.content,
...
async function getFirstPageData(that, pageSize) {
  const p1 = getPostingData(1, pageSize);
  const p2 = getTeamData(1, pageSize);

  Promise.all([p1, p2])
    .then(result => {
      const posting = result[0];
      const teams = result[1];
      const jishiData = dealWithPosting(posting);
      const jirenData = dealWithTeam(teams);
...
async function getNextPostingPage(that) {
  const { jishiIsLastPage, jishiCurrent, jishiPages } = that.data;
  if (!jishiIsLastPage) {
    try {
      const res = await getPostingData(jishiCurrent + 1);
      const postingData = dealWithPosting(res);
      that.setData({
...
async function getNextTeamPage(that) {
  const { jirenIsLastPage, jirenCurrent, jirenPages } = that.data;
  if (!jirenIsLastPage) {
    try {
      const res = await getTeamData(jirenCurrent + 1);
      const teamData = dealWithTeam(res);
      that.setData({
