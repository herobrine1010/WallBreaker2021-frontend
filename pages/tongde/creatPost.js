...
Page({
  ...
  tapOk: function () {
    ...
    const formValue = this.data.formValue; // 用户填写的表单数据
    const selectedLabels = this.data.selectedLabelList;

    if (this.WxValidate.checkForm({ ...formValue, selectedLabels })) {
      // 上传本地图片,拿到oss图片url
      const imagePromise = mergePathThenUploadImage(formValue.allPicPaths);
      wx.showLoading({
        title: "图片上传中",
        mask: true
      });
      imagePromise
        .then(res => {
          ...
          if (res.statusCode >= 200 && res.statusCode <= 300) {
            ...
            const firstImgURL = imageURL.split(",", 1)[0];
        ...
        .then(res => {
          if (res.statusCode >= 200 && res.statusCode <= 300 && res.data.success) {
            ...
            const lostFoundId = data.id; // 获取失物招领主键id, 之后添加标签
      ...
      const errors = {};
      for (const e of this.WxValidate.errorList) {
