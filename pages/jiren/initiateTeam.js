...
function getDateStringFromNow(days) {
  ...
  */
  const date = new Date();
  // date对象会自动进位，无需处理异常
  const futureDateNum = date.getDate() + days;
  ...
  const dateString = util.formatTime(date).split(" ")[0].replace(/[/]/g, "-");
  return dateString;
}
...
Page({
  ...
  onLoad() {
    ...
    wx.request({
      ...
      success: function (res) {
        const thememList = res.data.data;
        that.setData({
  ...
  tapOkForTeamSubmit: function (e) {
    ...
    const imagePath = this.selectComponent("#image-box").image; // 获取图片本地路径
      ...
      .then(res => {
        ...
        if (res.statusCode >= 200 && res.statusCode <= 300) {
          ...
          request({
            ...
            } else if (res.data.success && res.statusCode == 200) {
              // 有微信号，可以发起组队
              wx.showToast({
                title: "组队招募\n已发布成功",
                icon: "none",
