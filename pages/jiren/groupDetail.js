...
Page({
  ...
  onLoad: function (options) {
    ...
    const query = this.getTeamDetail(that.data.teamId);
    query
      .then(result => {
        // 先处理组队数据，再发请求入队成员列表
        ...
        return that.getTeamMemberList(
          that.data.teamId,
          that.data.amITeamInitiator,
          result.dueMember,
          that.data.applyStatus
        );
      })
      .then(teamMemberList => {
        ...
        if (that.data.amITeamInitiator) {
          return that.getApplierList(that.data.teamId).then(applierList => {
            that.setData({
              applierList
            });
          });
    ...
    return request({
      ...
      .then(res => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          const picList = allPicUrl ? allPicUrl.split(",") : [];
          const fromTime = util.getDateDiff(teamdata.createTime);
  ...
  checkStatus(teamData) {
    const that = this;
    const { reason, status, applyStatus, applyClosed, applyNotice, notice } = teamData;
    const amITeamInitiator = that.data.amITeamInitiator;
    //  ----- --------- -入队显示提示气泡：--- strat ------
  ...
  tapAvatar(e) {
    ...
    const avatar = that.data.teamMemberList[index];
    if (!avatar.description) {
      avatar.description = "这位同济er暂时没有话想说～";
    }
    const applyStatus = that.data.teamDetail.applyStatus;
  ...
  tapInitiatorAvatar() {
    const that = this;
    const teamDetail = that.data.teamDetail;
    const applyStatus = teamDetail.applyStatus;
    if (applyStatus == 1 || applyStatus == 2) {
  ...
  tapApplierAvatar(e) {
    const that = this;
    const index = e.currentTarget.dataset.index;
    const avatar = this.data.applierList[index];
    that.setData({
      isPersonalInfoShow: true,
      avatar
    });
  ...
  //   复制资料卡片的wxid:
  copyWxId() {
    const wxId = this.data.avatar.wxId;
    wx.setClipboardData({
      data: wxId
    });
  },
  ...
  dialogTapOkForCloseTeam: function (e) {
    const that = this;
    const reason = this.selectComponent("#dialogBox").data.reason;
    if (!reason) {
  ...
  showAnswers: function (e) {
    ...
    const that = this;
    const index = e.currentTarget.dataset.index;
    const applier = that.data.applierList[index];
    const answer = {
  ...
  // 在弹出的资料卡中点击查看回答
  showAnswersInPersonalAnimation: function (e) {
    // wxml页面上还存在点问题，待更改！！！！！！！！！！！！！！！！！！
    const that = this;
  ...
  acceptApplying: function (e) {
    const applyer = e.currentTarget.dataset;
    this.setData({
  ...
  dialogTapOkForAcceptApplying: function () {
    ...
    request({
      ...
      .then(res => {
        ...
        // 重新请求获取 成员列表、申请者列表数据
        const getTeamMemberList = that.getTeamMemberList(
          that.data.teamId,
          that.data.amITeamInitiator,
          that.data.teamDetail.dueMember
        );
        const getApplierList = that.getApplierList(that.data.teamId);
  ...
  dialogTapOkForRefuseApplying: function () {
    // 待解决 ： initializeResult 和 changeScrollHeight 作用
    const app = getApp();
    const that = this;
    request({
  ...
  seeDetail: function (e) {
    // 样式 待调整
    const teamDetail = this.data.teamDetail;
    const dialog2 = {
      isDialogShow: true,
  ...
  applyButton: function (e) {
    const that = this;
    const { notice, applyStatus, applyNotice, applyClosed } = that.data.teamDetail;
        ...
        wx.navigateTo({
          ...
          events: {
            ...
            getResult: function (data) {
              ...
              that
                .getTeamDetail(that.data.teamId)
                .then(result => {
                  that.setData({
                    teamDetail: result,
                    timeIsOver: result.status > 2 ? true : false
  ...
  changeSizeOfInitiatorPage() {
    ...
    const query = wx.createSelectorQuery();
    query
      .select("#initiator-scroll")
      .boundingClientRect(rect => {
        const top = rect.top;
        const height = this.data.windowHeight - top;
        this.setData({
  ...
  zoomInDetailPicture: function (e) {
    const imgUrl = this.data.teamDetail.pictures;
    wx.previewImage({
