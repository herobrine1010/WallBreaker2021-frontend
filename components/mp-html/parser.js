...
(s.prototype.parse = function (t) {
  ...
  (s.prototype.popNode = function () {
    ...
    if (t.name === "svg") {
      ...
      return (
        ...
        (function t(i) {
          ...
          const s = a.svgDict[i.name] || i.name;
          h += "<" + s;
          for (const e in i.attrs) {
            const n = i.attrs[e];
            n && (h += " ".concat(a.svgDict[e] || e, '="').concat(n, '"'));
          }
          if (i.children) {
            h += ">";
            for (let r = 0; r < i.children.length; r++) t(i.children[r]);
            h += "</" + s + ">";
          } else h += "/>";
        })(t),
        (t.name = "img"),
    ...
    const D = e && (e.attrs.style || "").includes("flex") && !t.c && !(c.display || "").includes("inline");
    D && (t.f = ";max-width:100%");
    for (const B in c) {
      if (c[B]) {
